...
export default function TaskList({ tasks, setShowModal, setEditTaskId, setTasks }: TaskListExtendedProps) {
	return (
		<div className="flex flex-col justify-center items-center py-8">
			...
			{tasks.length > 0 ? tasks.map((task) => <Task key={task.id} id={task.id} title={task.title} difficulty={task.difficulty} setShowModal={setShowModal} setEditTaskId={setEditTaskId} tasks={tasks} setTasks={setTasks} />) : <p>Não há tarefas no momento.</p>}
		</div>
	);
}
